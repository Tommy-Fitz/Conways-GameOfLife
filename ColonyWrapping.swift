struct ColonyWrapping: CustomStringConvertible{
  var colonySize: Int
  var colony: Array2DS
  init(colonySize: Int){
...
mutating func isCellAlive(xCoor: Int, yCoor: Int) -> Bool {
  if colony[xCoor,yCoor] == 1{
    return true
    } else{
     return false
  }
...
// if it has three neighbors then it is a corner and if it is the last row first coloum then you do first row first colom, first row last colum, and last row last colum, you also need to do the row minus one for one corner and colum minus one for the other
